...
import type { KeywordData, SearchSource, BlogPostData, KeywordMetrics, GeneratedTopic, BlogStrategyReportData, RecommendedKeyword, SustainableTopicCategory, GoogleSerpData, PaaItem, SerpStrategyReportData, WeatherData, NaverTrendKeyword, NaverTrendCategory, NaverTrendAnalysis, FrequentKeyword, BlogStrategySuggestion, NaverNewsData, NewsBlogStrategyReportData } from '../types';
import { GoogleGenAI, Type } from "@google/genai";
import { loadNaverApiKeys } from './apiKeys';
...
const fetchWithProxies = async (targetUrl: string, responseParser: (text: string) => any, options: { headers?: Record<string, string> } = {}) => {
    ...
    // If we're here, all proxies and retries have failed.
     if (lastKnownError instanceof Error) {
        ...
        if (lastKnownError instanceof TypeError && lastKnownError.message.includes('fetch')) {
         ...
         if (lastKnownError.message.includes('401')) {
            throw new Error('인증 실패(401). API 키가 잘못되었거나 권한이 없습니다.');
        }
        if (lastKnownError.message.includes('403')) {
            throw new Error('권한 없음(403). API 애플리케이션 설정을 확인하세요.');
        }
...
export const testNaverApiConnection = async (clientId: string, clientSecret: string): Promise<void> => {
    if (!clientId || !clientSecret) {
        throw new Error("클라이언트 ID와 시크릿을 모두 입력해야 합니다.");
    }
    const testUrl = `https://openapi.naver.com/v1/search/blog.json?query=test&display=1`;
    const headers = {
    ...
    try {
        await fetchWithProxies(testUrl, JSON.parse, { headers });
    } catch (error) {
        if (error instanceof Error) {
            if (error.message.includes('401')) {
                throw new Error('인증 실패. 클라이언트 ID 또는 시크릿이 잘못되었습니다.');
            }
            if (error.message.includes('403')) {
                throw new Error('권한 없음. API 애플리케이션 설정을 확인하세요.');
            }
        }
        throw new Error('API 엔드포인트에 연결할 수 없습니다. 프록시 또는 네트워크를 확인하세요.');
    }
};
...
export const fetchNaverNews = async (keyword: string): Promise<NaverNewsData[]> => {
    if (!keyword.trim()) {
        throw new Error("키워드가 비어있습니다.");
    }

    const apiKeys = loadNaverApiKeys();
    if (!apiKeys || !apiKeys.clientId || !apiKeys.clientSecret) {
        throw new Error("Naver 검색 API 키가 설정되지 않았습니다. 우측 상단 설정 아이콘을 클릭하여 API 키를 등록해주세요.");
    }
    
    const targetUrl = `https://openapi.naver.com/v1/search/news.json?query=${encodeURIComponent(keyword)}&display=10&sort=date`;
    ...
    try {
        const data = await fetchWithProxies(targetUrl, JSON.parse, { headers });
        
        if (!data || !Array.isArray(data.items)) {
            throw new Error("Naver API로부터 유효하지 않은 응답을 받았습니다.");
        }
        ...
        const results: NaverNewsData[] = data.items.map((item: any, index: number) => ({
            id: index + 1,
        ...
        if (error instanceof Error) {
            if (error.message.includes('401') || error.message.includes('403')) {
                 throw new Error("Naver API 인증에 실패했습니다. 설정에서 API 키가 올바른지 확인해주세요.");
            }
        }
        throw new Error(`Naver 뉴스 검색 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요.`);
    }
};

export const generateBlogStrategyFromNews = async (keyword: string, news: NaverNewsData[]): Promise<NewsBlogStrategyReportData> => {
    if (!keyword.trim()) throw new Error("분석할 키워드가 없습니다.");
    if (!news || news.length === 0) throw new Error("분석할 뉴스 데이터가 없습니다.");
    ...
    const prompt = `
당신은 15년차 SEO 전문가이자, 실시간 이슈를 활용한 콘텐츠 마케팅의 대가입니다.

다음은 '${keyword}' 키워드와 관련된 최신 뉴스 기사 목록입니다.

[최신 뉴스 목록]
${newsContext}

[지시사항]
1.  **핵심 이슈 분석**: 위 뉴스들을 종합하여, 현재 사용자들이 가장 궁금해하는 **핵심 쟁점(coreIssue)**, 사람들이 가장 관심을 보이는 **흥미 포인트(userInterestPoint)**, 그리고 아직 경쟁자들이 다루지 않은 **콘텐츠 기회(opportunity)**를 분석해 주세요. 분석 내용은 전문가적이고 매우 구체적이어야 합니다.
2.  **전략적 콘텐츠 제안**: 위 분석을 바탕으로, 검색 상위 노출을 달성하고 높은 클릭률을 유도할 수 있는, 시의성 높은 **블로그 제목, 썸네일 문구, 그리고 구체적인 공략법**을 10개 제안해 주세요.
...
아래 JSON 형식에 맞춰 **JSON 코드 블록 하나만으로** 응답해 주세요. 다른 설명은 절대 추가하지 마세요.
    ...
    const responseSchema = {
        ...
        properties: {
            analysis: {
                type: Type.OBJECT,
                properties: {
                    coreIssue: { type: Type.STRING, description: "뉴스 기사들을 관통하는 핵심 쟁점 또는 사건 요약" },
                    userInterestPoint: { type: Type.STRING, description: "사용자들이 이 이슈에 대해 가장 흥미를 느끼거나 궁금해하는 지점" },
                    opportunity: { type: Type.STRING, description: "경쟁 콘텐츠가 아직 다루지 않은, 선점 가능한 콘텐츠 기회" }
                },
                required: ['coreIssue', 'userInterestPoint', 'opportunity']
            },
            suggestions: {
                type: Type.ARRAY,
                items: {
    ...
    try {
        ...
        return parsed as NewsBlogStrategyReportData;
        ...
        if (error instanceof Error) {
            ...
            throw new Error(`뉴스 기반 블로그 전략 생성 중 AI 모델과 통신하는 데 실패했습니다. 오류: ${error.message}`);
        } else {
            console.error("알 수 없는 오류 발생:", error);
            throw new Error('뉴스 기반 블로그 전략 생성 중 알 수 없는 오류가 발생했습니다.');
        }
    }
};
...
export const fetchNaverBlogPosts = async (keyword: string): Promise<BlogPostData[]> => {
    ...
    const targetUrl = `https://openapi.naver.com/v1/search/blog.json?query=${encodeURIComponent(keyword)}&display=10&sort=sim`;
    ...
    try {
        ...
        const results: BlogPostData[] = data.items.map((item: any, index: number) => ({
            id: index + 1,
            title: stripHtml(item.title),
            url: item.link,
        }));
        ...
        if (results.length === 0) {
            ...
            // The UI will handle displaying the "no results" message.
            return [];
        }
        ...
        throw new Error(`Naver 블로그 검색 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요.`);
    }
};

export const naverTrendCategories: NaverTrendCategory[] = [
    { name: '맛집', slug: 'food' },
    { name: '국내여행', slug: 'trip' },
    { name: '세계여행', slug: 'w_trip' },
    { name: '패션·미용', slug: 'fashion' },
    { name: '상품리뷰', slug: 'review' },
    { name: '비즈니스·경제', slug: 'biz' },
    { name: '일상·생각', slug: 'daily' },
    { name: '육아·결혼', slug: 'parenting' },
    { name: '요리·레시피', slug: 'recipe' },
    { name: '건강·의학', slug: 'health' },
    { name: 'IT·컴퓨터', slug: 'tech' },
    { name: '교육·학문', slug: 'edu' },
    { name: '인테리어·DIY', slug: 'living' },
    { name: '자동차', slug: 'car' },
    { name: '스포츠', slug: 'sports' },
    { name: '취미', slug: 'hobby' },
    { name: '스타·연예인', slug: 'star' },
    { name: '방송', slug: 'broadcast' },
    { name: '게임', slug: 'game' },
    { name: '드라마', slug: 'drama' },
    { name: '영화', slug: 'movie' },
    { name: '공연·전시', slug: 'show' },
    { name: '반려동물', slug: 'animal' },
    { name: '음악', slug: 'music' },
    { name: '문학·책', slug: 'book' },
    { name: '사회·정치', slug: 'social' },
    { name: '어학·외국어', slug: 'lang' },
    { name: '만화·애니', slug: 'comic' },
    { name: '좋은글·이미지', slug: 'phrase' },
    { name: '원예·재배', slug: 'garden' },
    { name: '사진', slug: 'photo' },
    { name: '미술·디자인', slug: 'design' },
];

// Cache for Naver Trend data, per category, with a TTL
const naverTrendCache = new Map<string, NaverTrendKeyword[]>();
const naverTrendCacheTimestamp = new Map<string, number>();
const NAVER_TREND_CACHE_TTL_MS = 10 * 60 * 1000; // 10 minutes

export const fetchNaverTrendKeywords = async (categorySlug: string): Promise<NaverTrendKeyword[]> => {
    const now = Date.now();
    // Check cache first
    if (naverTrendCacheTimestamp.has(categorySlug) && (now - naverTrendCacheTimestamp.get(categorySlug)!) < NAVER_TREND_CACHE_TTL_MS) {
        const cachedData = naverTrendCache.get(categorySlug);
        if (cachedData) {
            return cachedData;
        }
    }

    const targetUrl = `https://creator-advisor.naver.com/creator/insight?service=blog&metric=popular_keywords&category=${categorySlug}`;
    
    const html = await fetchWithProxies(targetUrl, (text) => text);

    if (html.includes('<title>NAVER Login</title>') || html.includes('id="log.login"')) {
        // Clear cache for this slug if we hit a login wall, in case permissions change
        naverTrendCache.delete(categorySlug);
        naverTrendCacheTimestamp.delete(categorySlug);
        throw new Error('LOGIN_REQUIRED:네이버 로그인이 필요하여 트렌드 데이터를 가져올 수 없습니다. 프록시가 로그인 페이지를 반환했습니다.');
    }

    const parser = new DOMParser();
    const doc = parser.parseFromString(html, 'text/html');

    // Use selectors based on user's UserScript and inspection of the actual page structure.
    const rows = doc.querySelectorAll("li[data-testid='popular-keyword-item']");

    if (rows.length === 0) {
        console.warn(`Could not find trend keywords for category: ${categorySlug}. The page structure may have changed, or there is no data.`);
        // Cache the empty result to avoid re-fetching for a short period
        naverTrendCache.set(categorySlug, []);
        naverTrendCacheTimestamp.set(categorySlug, now);
        return [];
    }

    const keywords: NaverTrendKeyword[] = [];
    rows.forEach((row) => {
        const rankElement = row.querySelector("em[class*='_rank_']");
        const keywordElement = row.querySelector("span[class*='_keyword_']");
    ...
    // Save to cache
    naverTrendCache.set(categorySlug, keywords);
    naverTrendCacheTimestamp.set(categorySlug, now);

    return keywords;
};

export const analyzeNaverTrendCategory = async (categorySlug: string): Promise<NaverTrendAnalysis> => {
    const category = naverTrendCategories.find(c => c.slug === categorySlug);
    if (!category) {
        throw new Error(`'${categorySlug}'에 해당하는 카테고리를 찾을 수 없습니다.`);
    }
    const categoryName = category.name;

    // Step 1: Fetch top 10 blog posts for the CATEGORY NAME.
    const blogPosts = await fetchNaverBlogPosts(categoryName);

    if (!blogPosts || blogPosts.length === 0) {
        return {
            blogPosts: [],
            frequentKeywords: [],
            keywordBasedSuggestions: [],
            analysisBasedSuggestions: []
        };
    }
    
    // Step 2, 3, 4: Use Gemini to analyze titles, get keywords, and get suggestions.
    const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
    const blogTitles = blogPosts.map((post, i) => `${i + 1}. ${post.title}`).join('\n');

    const prompt = `
    당신은 10년차 SEO 전문가이자, 사용자의 클릭을 유도하는 콘텐츠 마케팅 전문가입니다.
    다음은 '${categoryName}' 키워드로 검색했을 때 상위 10개에 노출된 블로그 포스팅 제목 목록입니다.

    [상위 10개 블로그 제목]
    ${blogTitles}

    [지시사항]
    당신의 임무는 위 데이터를 종합적으로 분석하여, 경쟁을 이기고 1위를 차지할 수 있는 완벽한 콘텐츠 전략을 수립하는 것입니다. 아래 3가지 분석을 수행하고, 결과물을 JSON 형식으로 반환해 주세요.

    1.  **핵심 키워드 분석**: 위 제목들에서 가장 빈번하게 사용된 **핵심 명사 키워드**를 분석하여, 빈도수와 함께 상위 10개를 추출합니다. (예: "제주", "맛집", "카페"). 일반적인 단어(예: '추천', '방법', '후기')는 제외합니다.
    2.  **키워드 기반 제목 제안**: 1번에서 추출한 **핵심 키워드들을 전략적으로 조합**하여, 검색 상위 노출에 유리하고 사용자의 클릭을 유도할 수 있는 새로운 블로그 제목 5개를 제안합니다. 각 제안에는 제목, 썸네일 문구, 구체적인 공략법을 포함해야 합니다.
    3.  **상위 제목 구조 분석 기반 제목 제안**: 상위 10개 제목들의 **구조적(숫자, 질문형 등), 감성적(호기심, 신뢰 등) 특징을 분석**하고, 그 분석 결과를 바탕으로 기존 포스팅들을 능가할 수 있는 더 매력적인 블로그 제목 5개를 제안합니다. 각 제안에는 제목, 썸네일 문구, 구체적인 공략법을 포함해야 합니다.

    [출력 형식]
    - 다른 설명 없이 오직 아래 JSON 형식에 맞춰 **JSON 코드 블록 하나만으로** 응답해 주세요.
    `.trim();
    ...
    const suggestionSchema = {
        ...
        items: {
            ...
            properties: {
                title: { type: Type.STRING, description: "새로운 블로그 제목" },
                thumbnailCopy: { type: Type.STRING, description: "썸네일에 사용할 짧고 강력한 문구" },
                strategy: { type: Type.STRING, description: "이 제목이 왜 효과적인지에 대한 구체적인 공략법" }
            },
            required: ['title', 'thumbnailCopy', 'strategy']
        }
    };
    ...
    const responseSchema = {
        ...
        properties: {
            frequentKeywords: {
                type: Type.ARRAY,
                description: "분석된 핵심 키워드와 빈도수 목록 (빈도수 높은 순)",
                items: {
                    type: Type.OBJECT,
                    properties: {
            ...
            keywordBasedSuggestions: {
                ...suggestionSchema,
                description: "추출된 키워드를 조합하여 만든 블로그 제목 제안 5개"
            },
            analysisBasedSuggestions: {
                ...suggestionSchema,
                description: "상위 제목 구조를 분석하여 만든 블로그 제목 제안 5개"
            },
        },
        required: ['frequentKeywords', 'keywordBasedSuggestions', 'analysisBasedSuggestions']
    };
    ...
    try {
        ...
        if (!parsed.frequentKeywords || !parsed.keywordBasedSuggestions || !parsed.analysisBasedSuggestions) {
            throw new Error("AI 응답이 유효한 형식이 아닙니다.");
        }
        
        const keywordSuggestions: BlogStrategySuggestion[] = parsed.keywordBasedSuggestions.map((s: any, i: number) => ({ ...s, id: i + 1 }));
        const analysisSuggestions: BlogStrategySuggestion[] = parsed.analysisBasedSuggestions.map((s: any, i: number) => ({ ...s, id: i + 1 + keywordSuggestions.length }));

        return {
            blogPosts,
            frequentKeywords: parsed.frequentKeywords,
            keywordBasedSuggestions: keywordSuggestions,
            analysisBasedSuggestions: analysisSuggestions,
        };
    } catch (error) {
        console.error("Naver 트렌드 심층 분석 중 Gemini API 오류:", error);
        return {
            blogPosts,
            frequentKeywords: [],
            keywordBasedSuggestions: [],
